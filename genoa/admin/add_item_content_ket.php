<?php 
            ...
            if (count($error_array) == 0) {
                run_q('INSERT INTO items_ket (item_name,item_weight,cat_id,item_price,pic_name) 
                       VALUES ("'.$post_item_name.'", '.$post_item_weight.','.$post_pselect_group.', '.$post_item_price.', "'.$post_image_name.'")');
                if(!mysql_error()){redirect('add_item_ket.php?succ_add=1');}else {
                    mysql_error();
                }
            }
            else {
        ...
        if(isset ($_POST['edit_post']) && isset ($_POST['edit_post_id']) && $_POST['edit_post']>0){
            ...
            if($edit_id_post>0){
                $result_edit_post = run_q('SELECT item_id FROM items_ket WHERE item_id='.$edit_id_post);
                ...
                if($edit_num_rows_post>0){
                    ...
                    // run_q
                    if (count($error_array) == 0) {
                        run_q('UPDATE items_ket SET item_name="'.$post_item_name.'",item_weight="'.$post_item_weight.'",cat_id='.$post_pselect_group.',item_price="'.$post_item_price.'",pic_name="'.$post_image_name.'" WHERE item_id='.$edit_id_post);
                        ...
                        redirect('add_item_ket.php?editid='.$edit_id_post.'&succ_edit=1');
                    }
                    else {
                        mysql_error();
                    }
        ...
        if(isset($_GET['editid'])){
            ...
            $edit_result = run_q('SELECT * FROM items_ket WHERE item_id='.$editid);
...
if(isset($_GET['deleteid'])){
            ...
            if($deleteid>0){
                $result_delete = run_q('SELECT item_id,pic_name FROM `items_ket` WHERE item_id='.$deleteid);
                ...
                if($delete_num_rows>0){
                        ...
                        run_q('DELETE FROM `items_ket` WHERE item_id='.$deleteid);
                        redirect('add_item_ket.php?succ_del=1');
                }
...
<?php 
    $group_result = run_q('SELECT `cat_id`,`ctitle` FROM `menu_ket`'); // DATABASE CONNECT
