...
$result = run_q('SELECT * FROM items_ket '.$group_url.' ORDER BY item_id DESC ');
?>
            <div id="adm_table_field">
                <p><strong>Покажи:</strong> 
                    <a href="menu_ketering_adm.php">Всички</a> | 
                    <?php 
                        $group_url_result = run_q('SELECT cat_id,ctitle FROM `menu_ket`');
                        while ($group_url_row = mysql_fetch_array($group_url_result)){
                            echo '<a href="menu_ketering_adm.php?group='.$group_url_row['cat_id'].'">'.$group_url_row['ctitle'].'</a> | ';
                        }
                    ?>
                <table>
                    <tr class="tbl_head">
...
<?php 
                    while ($row = mysql_fetch_array($result)) {
                        $group_result = run_q('SELECT `ctitle` FROM `menu_ket` WHERE cat_id='.$row['cat_id']);
                        ...
                        <td><a href="../menu_ketering.php" class="icon_btn"><img src="../img/icons/preview.png"/></a><a href="add_item_ket.php?editid='.$row['item_id'].'" class="icon_btn"><img src="../img/icons/edit.png"/></a><a onClick="return question();" href="add_item_ket.php?deleteid='.$row['item_id'].'" class="icon_btn"><img src="../img/icons/delete.png"/></a></td>
                    </tr>';
                    }
?>                    
<script type="text/javascript" language="javascript">
    function question(){
        ...
        if (quest){return true;} else{return false;}
                ...
                <a id="add_product_btn" href="add_item_ket.php">Добави нов продукт!</a>
            </div>
