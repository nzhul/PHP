<?php 
$result = run_q('SELECT * FROM `group`');
?>
<?php 
if(isset ($_POST['new_post']) && $_POST['new_post']>0){
            $error_array = array();
            $post_gname = $_POST['gname'];
            $post_gdesc = $_POST['gdesc'];
            
            if(mb_strlen($post_gname,'UTF-8')<4){
                $error_array['post_gname'] = '<span style="color:red;font-weight:bold;">Името на групата</span> е твърде кратко!';
            }
            if(mb_strlen($post_gname,'UTF-8')>200){
                $error_array['post_gname'] = '<span style="color:red;font-weight:bold;">Името на групата</span> е твърде дълго - макс 200!';
            }
            if(mb_strlen($post_gdesc,'UTF-8')<4){
                $error_array['post_gdesc'] = '<span style="color:red;font-weight:bold;">Описанието на групата</span> е твърде кратко!';
            }
            if(mb_strlen($post_gdesc,'UTF-8')>20000){
                $error_array['post_gdesc'] = '<span style="color:red;font-weight:bold;">Описанието на групата</span> е твърде дълго - макс 20 000!';
            }
            if (count($error_array) == 0) {
                run_q('INSERT INTO `group` (gname,gdesc,date_added) VALUES ("'.$post_gname.'","'.$post_gdesc.'",'.time().')');
...
if(isset ($_POST['edit_post']) && isset ($_POST['edit_post_id']) && $_POST['edit_post']>0){
            ...
            $post_gdesc = $_POST['gdesc'];
            if($edit_id_post>0){
                $result_edit_post = run_q('SELECT group_id FROM `group` WHERE group_id='.$edit_id_post);
                ...
                if($edit_num_rows_post>0){
                    ...
                    if (count($error_array) == 0) {
                        run_q('UPDATE `group` SET gname="'.$post_gname.'",gdesc="'.$post_gdesc.'" WHERE group_id='.$edit_id_post);
...
if(isset($_GET['editid'])){
    ...
    $edit_post_id = $editid;
    $edit_result = run_q('SELECT * FROM `group` WHERE group_id='.$editid);
    $edit_row = mysql_fetch_array($edit_result);
    $gname = $edit_row['gname'];
    $gdesc = $edit_row['gdesc'];
    $btn_value = 'Запази';
}
else {
...
if(isset($_GET['deleteid'])){
            ...
            if($deleteid>0){
                $result_delete = run_q('SELECT group_id FROM `group` WHERE group_id='.$deleteid);
                $delete_num_rows = mysql_num_rows($result_delete);
                if($delete_num_rows>0){
                        run_q('UPDATE products SET group_id=9 WHERE group_id='.$deleteid); // Сменям групата на всички продукти на - 9 - "ДРУГИ"
                        run_q('DELETE FROM `group` WHERE group_id='.$deleteid); // След това изтривам групата
...
<?php 
                    while ($row = mysql_fetch_array($result)) {
                        $short_desc = mb_substr($row['gdesc'], 0, 400, 'UTF-8');
                        echo '<tr>
                        <td style="width:180px;font-weight:bold;">'.$row['gname'].'</td>
                        <td>'.$short_desc.'</td>
                        <td style="vertical-align:middle;"><a href="../products.php?group='.$row['group_id'].'" class="icon_btn"><img src="../img/icons/preview.png"/></a><a href="categories.php?editid='.$row['group_id'].'#formm" class="icon_btn"><img src="../img/icons/edit.png"/></a><a onClick="return question();" href="categories.php?deleteid='.$row['group_id'].'" class="icon_btn"><img src="../img/icons/delete.png"/></a></td>
                    </tr>';
                    }
?>                    <tr class="tbl_head">
                        <td>Продукт</td>
